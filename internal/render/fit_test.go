...
func TestWriteContextFitPlain(t *testing.T) {
	...
	WriteContextFit(&out, ContextFit{
		HaveGB: 20,
		Note:   "weights + KV only; other runtime allocation not observed until --load or --fit",
	...
	got := out.String()
	for _, want := range []string{"4096", "32768", "compatible", "incompatible", "n/a", "24.8", "suggested", "other n/a"} {
