...
func FuzzConversionManifest(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, body string) {
		...
		for _, a := range m.Artifacts {
			...
			if seen[a.Digest] {
				t.Fatalf("accepted a manifest repeating artifact %s", a.Digest)
			}
			seen[a.Digest] = true
			if strings.TrimSpace(a.Digest) != a.Digest {
				t.Fatalf("digest %q was not normalized", a.Digest)
			}
		}
