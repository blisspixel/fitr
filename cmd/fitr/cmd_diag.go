...
func cmdDoctor(ctx context.Context, args []string) int {
	...
	model := normalizeModelRef(command.model)
	c, code := newBackend(ctx, model, command.backend, false)
	if code != exitOK {
		return code
	}
	return runDoctor(ctx, c, model, command)
}
...
func parseDoctorCommand(args []string) (doctorCommand, int, bool) {
	...
	ctxSize := fs.Int("ctx", 0, "request context (default 8192)")
	if code, ok := parseCommandFlags(fs, args); !ok {
		return doctorCommand{}, code, false
	}
	if fs.NArg() < 1 {
		errPrint("missing model", "", "fitr doctor <model>")
		return doctorCommand{}, exitUsage, false
	}
	if fs.NArg() > 1 {
	...
	return doctorCommand{model: fs.Arg(0), backend: *backend, repeats: *n, numCtx: *ctxSize}, exitOK, true
}

func runDoctor(ctx context.Context, c llm.Backend, model string, command doctorCommand) int {
